...
.. important::
    ...
    任一必要引用无法解析时，任务依赖检查失败，不占用令牌。
    定义组装完成后，可调用 ``flow.validate_limit_references()`` 或
    ``bunch.validate_limit_references()`` 获取声明节点、限额名与引用路径。
    构建过程中的 ``add_*`` 操作不做提前校验；begin 也不自动拒绝缺失引用。
    同一 bunch 内可使用绝对路径引用其他 flow 的限额。
...
* 任务进入 ``submitted`` 或直接进入 ``active`` 时占用令牌；重复进入不重复占用
