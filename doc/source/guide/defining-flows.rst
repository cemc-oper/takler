...
每次 ShellScriptTask 提交都会创建独立的进程内标识。异步失败回调只有在
该标识仍属于当前提交、节点仍位于原树的原路径且状态仍为 submitted/active
时，才会把任务置为 aborted。requeue、再次运行以及离开运行状态都会使
旧回调失效，因此 requeue 后 try_no 再次为 1 也不会混淆两次提交。
已经从在线 Bunch 移除或替换的节点，其旧回调不会修改旧节点或新树。

该标识不包含作业口令，不进入定义、查询或 checkpoint，也不提供持久化
