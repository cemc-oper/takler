...
def test_add_parameter_with_single_value():
    ...
    assert node.user_parameters["ECF_HOME"] == Parameter(
        name="ECF_HOME", value="/home/johndoe"
    )
    ...
    assert node.user_parameters["TIME_INTERVAL"] == Parameter(
        name="TIME_INTERVAL", value=0.1
    )

    node.add_parameter("FLAG_UPLOAD", True)
    assert node.user_parameters["FLAG_UPLOAD"] == Parameter(
        name="FLAG_UPLOAD", value=True
    )
...
def test_add_parameter_with_invalid_value_type():
    ...
    with pytest.raises(TypeError):
        node.add_parameter(
            "TYPHOON_INFO", dict(typhoon_id="W2501", typhoon_name="TYPH1")
        )
