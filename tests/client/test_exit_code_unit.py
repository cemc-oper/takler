...
def test_constants():
    assert (EXIT_OK, EXIT_REQUEST_ERROR, EXIT_SERVER_ERROR, EXIT_UNREACHABLE) == (0, 1, 3, 4)
...
def test_unregistered_takler_subclass_falls_back_to_generic_exit_code():
    ...
    assert exit_code_for_exception(exc) == exit_code_for_error_code(GENERIC_TAKLER_ERROR)
...
def test_foreign_exception_is_a_server_error(exc):
    ...
    assert exit_code_for_exception(exc) == exit_code_for_error_code(INTERNAL_SERVER_ERROR)
...
def test_error_code_column_covers_the_whole_error_code_table():
    ...
    assert set(EXIT_CODE_BY_ERROR_CODE.values()) <= {
        EXIT_OK, EXIT_REQUEST_ERROR, EXIT_SERVER_ERROR, EXIT_UNREACHABLE
    }
    # Only success maps to exit code 0.
    assert [code for code, exit_code in EXIT_CODE_BY_ERROR_CODE.items()
            if exit_code == EXIT_OK] == [SUCCESS]
...
def test_exit_code_module_only_depends_on_takler_exceptions():
    ...
    for node in ast.walk(ast.parse(source)):
        if isinstance(node, ast.ImportFrom) and (node.module or "").startswith("takler"):
